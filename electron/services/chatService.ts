...
class ChatService {
    ...
    try {
      ...
      for (const row of rows) {
        ...
        if (messageCountHint === undefined) {
          ...
          if (cachedStats && cachedStats.entry && cachedStats.entry.stats && cachedStats.entry.stats.totalMessages) {
            messageCountHint = cachedStats.entry.stats.totalMessages
          }
        }
  ...
  async getMessages(
    ...
    try {
        ...
        if (offset > 0) {
          ...
          while (skipped < offset && attempts < maxSkipAttempts) {
            ...
            if (!skipBatch.rows || skipBatch.rows.length === 0) {
              ...
              if (skipped === 0) {
                const fallbackResult = startTime === 0 && endTime === 0 && !ascending
                  ? await this.getMessagesByOffsetStable(sessionId, offset, requestLimit)
                  : await this.getMessagesByCursorScanStable(sessionId, offset, requestLimit, startTime, endTime, ascending)
                if (fallbackResult.success && Array.isArray(fallbackResult.messages)) {
                  await this.closeMessageCursorBySession(sessionId)
                  releaseMessageCursorMutex?.()
                  console.warn(
                    `[ChatService] 游标跳过异常，已切换稳定扫描兜底: session=${sessionId}, offset=${offset}, startTime=${startTime}, endTime=${endTime}, ascending=${ascending}, returned=${fallbackResult.messages.length}, hasMore=${fallbackResult.hasMore === true}`
                  )
                  return {
                    success: true,
      ...
      if (offset === 0 && startTime === 0 && endTime === 0 && !ascending) {
  ...
  private async getMessagesByCursorScanStable(
    sessionId: string,
    offset: number,
    limit: number,
    startTime: number,
    ...
    const pageLimit = Math.max(1, Math.floor(limit || this.messageBatchDefault))
    const safeOffset = Math.max(0, Math.floor(offset || 0))
    const batchSize = Math.min(500, Math.max(120, pageLimit + safeOffset + 1))
    const beginTimestamp = startTime > 10000000000 ? Math.floor(startTime / 1000) : Math.max(0, Math.floor(startTime || 0))
    const endTimestamp = endTime > 10000000000 ? Math.floor(endTime / 1000) : Math.max(0, Math.floor(endTime || 0))
    const cursorResult = await wcdbService.openMessageCursor(sessionId, batchSize, ascending, beginTimestamp, endTimestamp)
    if (!cursorResult.success || !cursorResult.cursor) {
      return { success: false, error: cursorResult.error || '打开稳定扫描游标失败' }
    }

    const wantedRawRows = safeOffset + pageLimit + 1
    const rawRows: Record<string, any>[] = []
    let hasMoreFromCursor = false
    let emptyBatchStreak = 0
    try {
      while (rawRows.length < wantedRawRows) {
        const batch = await wcdbService.fetchMessageBatch(cursorResult.cursor)
        if (!batch.success) {
          return { success: false, error: batch.error || '稳定扫描消息失败' }
        }

        const rows = Array.isArray(batch.rows) ? batch.rows as Record<string, any>[] : []
        if (rows.length === 0) {
          if (batch.hasMore && emptyBatchStreak < 2) {
            emptyBatchStreak += 1
            continue
          }
          hasMoreFromCursor = batch.hasMore === true
          break
        }
        ...
        rawRows.push(...rows)
        if (!batch.hasMore) {
          hasMoreFromCursor = false
          break
        }
        hasMoreFromCursor = true
      }
    } finally {
      await wcdbService.closeMessageCursor(cursorResult.cursor).catch(() => {})
    }

    const hasMore = rawRows.length > safeOffset + pageLimit || hasMoreFromCursor
    const selectedRows = rawRows.slice(safeOffset, safeOffset + pageLimit)
    const mapped = this.mapRowsToMessages(selectedRows, sessionId)
    const visible = mapped.filter((msg) => this.isMessageVisibleForSession(sessionId, msg))
    const outputMessages = (visible.length === 0 && mapped.length > 0)
      ? mapped
      : visible
    if (visible.length === 0 && mapped.length > 0) {
      console.warn(`[ChatService] getMessagesByCursorScanStable 可见性过滤回退: session=${sessionId} mapped=${mapped.length}`)
    }
    const normalized = this.normalizeMessageOrder(outputMessages)
    if (normalized.length > 0) {
      await this.repairEmojiMessages(normalized)
  ...
  async getNewMessages(
    sessionId: string,
    minTime: number,
    limit: number = this.messageBatchDefault,
    cursor?: { createTime?: number; sortSeq?: number; localId?: number; serverId?: number | string; serverIdRaw?: string }
    ...
    try {
      ...
      const requestLimit = Math.max(1, Math.floor(limit || this.messageBatchDefault))
      if (cursor) {
        return await this.getNewMessagesAfterCursor(sessionId, minTime, requestLimit, cursor)
      }

      const res = await wcdbService.getNewMessages(sessionId, minTime, requestLimit)
  ...
  private async getNewMessagesAfterCursor(
    sessionId: string,
    minTime: number,
    limit: number,
    cursor: { createTime?: number; sortSeq?: number; localId?: number; serverId?: number | string; serverIdRaw?: string }
    ...
    const cursorMessage = {
      messageKey: '',
      localId: Number(cursor.localId || 0),
      serverId: Number(cursor.serverIdRaw ?? cursor.serverId ?? 0),
      serverIdRaw: cursor.serverIdRaw !== undefined ? String(cursor.serverIdRaw) : undefined,
      localType: 0,
      createTime: Number(cursor.createTime || 0),
      sortSeq: Number(cursor.sortSeq || 0),
      isSend: null,
      senderUsername: null,
      parsedContent: ''
    } as Message
    const batchSize = Math.min(500, Math.max(120, limit + 32))
    const beginTimestamp = minTime > 10000000000 ? Math.floor(minTime / 1000) : Math.max(0, Math.floor(minTime || 0))
    const cursorResult = await wcdbService.openMessageCursor(sessionId, batchSize, true, beginTimestamp, 0)
    if (!cursorResult.success || !cursorResult.cursor) {
      return { success: false, error: cursorResult.error || '打开增量消息游标失败' }
    }
    ...
    try {
      while (collected.length < limit) {
        const batch = await wcdbService.fetchMessageBatch(cursorResult.cursor)
        if (!batch.success) {
          return { success: false, error: batch.error || '获取增量消息失败' }
        }
        ...
        const messages = this.normalizeMessageOrder(this.mapRowsToMessages(rows, sessionId))
        for (const message of messages) {
          if (this.compareMessagesByTimeline(message, cursorMessage) <= 0) continue
    ...
    try {
      ...
      switch (referType) {
        ...
        case '49': {
          const decodedReferContent = this.decodeHtmlEntities(referContent || '')
          const innerInfo = this.parseType49Message(decodedReferContent)
          displayContent = innerInfo.linkTitle || '[链接]'
          break
        }
        case '42':
          displayContent = '[名片]'
  ...
  async resolveQuotedMessages(messages: Message[], sessionId: string): Promise<void> {
    ...
    const chunkSize = 8
    for (let index = 0; index < uniqueSvrids.length; index += chunkSize) {
      const chunk = uniqueSvrids.slice(index, index + chunkSize)
  ...
  async searchMessages(keyword: string, sessionId?: string, limit?: number, offset?: number, beginTimestamp?: number, endTimestamp?: number): Promise<{ success: boolean; messages?: Message[]; error?: string }> {
    try {
      ...
      const rows = result.messages as Record<string, any>[]
      const parseChunkSize = 16

      for (let index = 0; index < rows.length; index += parseChunkSize) {
        const chunk = rows.slice(index, index + parseChunkSize)
        const parsedChunk = await Promise.all(chunk.map(async (row) => {
